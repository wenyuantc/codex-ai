...
export function buildSessionContext(ctx, platformInput = null) {
  const directory = ctx.directory;
  const contextKey =
    typeof ctx.getContextKey === "function" ? ctx.getContextKey(platformInput) : null;
...
Trellis compact SessionStart context. Use it to orient the session; load details on demand.
  ...
  parts.push("Discover more via: " + "`python3 ./.trellis/scripts/get_context.py --mode packages`");
  parts.push("</guidelines>");
