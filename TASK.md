...
- [ ] i18n 完整框架（本波仅加强空态 CTA；不做抽取式多语言）
- [ ] 更强报表与外部 Issues 同步（Issues 属明确不做；报表可另立）
...
- 员工默认是在线状态，修改员工的时候可以修改员工在线或者离线，离线的员工不能运行任务（**需复核**：前端有状态 UI，后端启动任务路径未见统一硬拦截；暂不排期，确认缺口后再提升）
