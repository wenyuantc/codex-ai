...
export function AboutUpdateSection() {
  ...
  const handleInstall = async () => {
    ...
    try {
      ...
      const previousVersion = currentVersion ?? availableUpdate.currentVersion;
      try {
        await logActivity({
          action: "app_update_installed",
          details: `${previousVersion} → ${availableUpdate.version}`,
        });
      } catch (logError) {
        console.error("Failed to log app update activity:", logError);
      }
      setInstalled(true);
    } catch (cause) {
      setMappedError(cause);
  ...
  return (
    <div className="space-y-4 rounded-lg border border-border bg-card p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h3 className="mb-1 text-sm font-medium">{t("about.title")}</h3>
      ...
      {upToDate && !availableUpdate ? (
        <p className="text-sm text-muted-foreground">{t("about.upToDate")}</p>
      ) : null}
