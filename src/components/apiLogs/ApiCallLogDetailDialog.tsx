...
import { MonacoJsonPreview } from "@/components/apiLogs/MonacoJsonPreview";
...
export function ApiCallLogDetailDialog({
  open,
  logId,
  environmentMode,
  selectedSshConfigId,
  ...
  useEffect(() => {
    ...
    void getNativeApiCallLog(logId, {
      environment_mode: environmentMode ?? null,
      selected_ssh_config_id: selectedSshConfigId ?? null,
      project_id: projectId ?? null,
      execution_target: environmentMode ?? null,
    })
      .then((result) => {
        if (!active) {
          return;
        ...
        {loading ? (
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-auto pr-1">
            ...
            <div className="grid gap-3 rounded-xl border border-border/70 p-3 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <MetaItem label={t("project")} value={detail.project_name?.trim() || unknown} />
              <MetaItem label={t("employee")} value={detail.employee_name?.trim() || unknown} />
              <MetaItem label={t("taskId")} value={detail.task_id?.trim() || unknown} />
...
function JsonBodySection({
  ...
  return (
    <div className="space-y-2">
      ...
      {body ? (
        <MonacoJsonPreview value={body} className="h-64" />
      ) : (
        <div
          className={cn(
