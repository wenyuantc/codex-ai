...
import {
  listNativeApiCallLogs,
  logActivity,
  type NativeApiCallLogListItem,
  type NativeApiCallLogStats,
} from "@/lib/backend";
...
export function ApiCallLogsPage() {
  const { t } = useTranslation(["apiLogs", "common"]);
    ...
    async (silent = false, requestedPage = 1) => {
      ...
      try {
        const result = await listNativeApiCallLogs({
          ...
          selected_ssh_config_id: environmentMode === "ssh" ? (selectedSshConfigId ?? null) : null,
  ...
  useEffect(() => {
    ...
    void logActivity({
      action: "api_call_logs_viewed",
      details: t("activityViewed"),
      project_id: currentProjectId ?? null,
    }).catch(() => undefined);
  }, [currentProjectId, t]);
  ...
  return (
    <>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-xl border border-border/70 p-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-8">
                ...
                <StatCell
                  label={t("statsInput")}
                  value={formatApiCallLogTokenCount(stats.input_tokens_sum, unknown)}
                />
                <StatCell
                  label={t("statsOutput")}
                  value={formatApiCallLogTokenCount(stats.output_tokens_sum, unknown)}
                />
                <StatCell
                  label={t("statsCached")}
      ...
      <ApiCallLogDetailDialog
        ...
        selectedSshConfigId={environmentMode === "ssh" ? (selectedSshConfigId ?? null) : null}
