...
use crate::app::{
    fetch_ssh_config_record_by_id, normalize_runtime_path_string, sqlite_pool,
...
pub(super) async fn resolve_task_project_execution_context<R: Runtime>(
    ...
    .await
    .map_err(|error| {
        format!("Failed to resolve task {} project execution context: {}", task_id, error)
    })?
    ...
    if project_type == EXECUTION_TARGET_SSH {
        let ssh_config_id = ssh_config_id
            .ok_or_else(|| "当前 SSH 项目缺少 ssh_config_id，无法启动 Claude。".to_string())?;
        ...
        let working_dir = remote_repo_path
            ...
            .ok_or_else(|| "当前 SSH 项目缺少远程仓库目录，无法启动 Claude。".to_string())?;
        Ok(ExecutionContext {
        ...
        Some(context) => {
            if context.execution_target == EXECUTION_TARGET_LOCAL {
                match context.working_dir.as_deref() {
                    ...
                    None => Err("当前任务所属项目未配置仓库路径，无法启动 Claude。".to_string()),
                }
            } else {
                Ok(context)
            }
