...
fn build_project_git_commit_file_preview_local(
    ...
    let after_snapshot = if normalized_change_type == "deleted" {
        ...
        capture_revision_text_snapshot_local(&runtime.repo_path, &trimmed_commit_sha, trimmed_path)
    };
...
fn cleanup_worktree_force_remove_is_allowed_after_failure() {
    ...
    assert!(
        !action_allows_failed_context("merge", &serde_json::json!({ "force_remove": true }))
...
fn prepare_task_git_execution_prefers_current_branch_over_origin_head() {
    tauri::async_runtime::block_on(async {
        ...
        let feature_head = run_git_text(&repo_path, &["rev-parse", "HEAD"]).expect("feature head");
        ...
        let worktree_head =
            run_git_text(&context.worktree_path, &["rev-parse", "HEAD"]).expect("worktree head");
...
fn merge_action_merges_task_branch_into_target_branch() {
    tauri::async_runtime::block_on(async {
        ...
        let main_before = run_git_text(&repo_path, &["rev-parse", "main"]).expect("main before");
        let task_head =
...
fn project_git_commit_file_preview_covers_root_modified_added_renamed_and_deleted() {
    ...
    let modified_commit = run_git_text(&repo_path, &["rev-parse", "HEAD"]).expect("modified sha");
    ...
    run_git_command(&repo_path, &["commit", "-q", "-m", "add feature"]).expect("commit added file");
