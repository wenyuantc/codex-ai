...
use crate::app::{
    fetch_project_by_id, fetch_ssh_config_record_by_id, normalize_runtime_path_string, sqlite_pool,
    validate_project_repo_path, ARTIFACT_CAPTURE_MODE_LOCAL_FULL, ARTIFACT_CAPTURE_MODE_SSH_FULL,
...
pub async fn resolve_task_project_execution_context<R: Runtime>(
    app: &AppHandle<R>,
    task_id: &str,
    engine_label: &str,
...
pub async fn resolve_one_shot_working_dir<R: Runtime>(
    ...
    let execution_context = match task_id.map(str::trim).filter(|value| !value.is_empty()) {
        Some(task_id) => {
            Some(resolve_task_project_execution_context(app, task_id, engine_label).await?)
        }
        None => match project_id.map(str::trim).filter(|value| !value.is_empty()) {
            Some(project_id) => {
                Some(resolve_project_execution_context(app, project_id, engine_label).await?)
            }
            None => None,
        },
    };
        ...
        Some(context) => {
            if context.execution_target == EXECUTION_TARGET_LOCAL {
                match context.working_dir.as_deref() {
                    ...
                    None => Err(format!(
                        "当前任务所属项目未配置仓库路径，无法启动 {engine_label}。"
                    )),
                }
            } else {
                Ok(context)
...
mod tests {
    ...
    #[test]
    fn engine_label_appears_in_missing_repo_path_message() {
