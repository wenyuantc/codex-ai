...
function normalizeProviderModel(provider, providerKey, modelId, modelInfo) {
  const providerValue = provider?.id || provider?.providerId || provider?.providerID || providerKey;
  const rawModelId = modelId || modelInfo?.id || modelInfo?.modelId || modelInfo?.modelID || "";
  const value = modelInfo?.value || (providerValue ? `${providerValue}/${rawModelId}` : "");
...
async function fetchMessageCursor(client, sessionId, config) {
  try {
    ...
    return {
      ...
      latestCreated: messages.reduce((latest, item) => Math.max(latest, messageCreated(item)), 0),
    };
  } catch {
  ...
  try {
    ...
    const runRequestedMode = async () => {
      if (mode === "list-providers") {
        ...
        for (const [providerKey, provider] of providerEntries(providers)) {
          ...
          for (const [modelId, modelInfo] of modelEntries(provider?.models || provider?.model)) {
    ...
    try {
      ...
      if (connectedExistingServer && isOpencodeConfigContentError(error)) {
        emit("info", {
          message: "运行中的 OpenCode server 使用旧版无效配置，已切换到隔离临时实例重试...",
        });
