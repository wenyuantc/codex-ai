...
use super::{GrokChild, GrokManager, GrokSessionKind, STOP_WAIT_MAX_ATTEMPTS, STOP_WAIT_POLL_MS};
...
async fn bind_grok_session_id(
    ...
    if let Ok(pool) = sqlite_pool(app).await {
        let _ = insert_codex_session_event(
            ...
            session_record_id,
            "cli_session_bound",
            Some(&format!("Grok CLI 会话已绑定: {}", cli_session_id)),
        )
        .await;
    }
...
async fn emit_grok_output_line(
    app: &AppHandle,
...
pub(super) fn spawn_grok_session_runtime(
    app: AppHandle,
    manager_state: Arc<tokio::sync::Mutex<GrokManager>>,
    child_arc: Arc<tokio::sync::Mutex<GrokChild>>,
    ...
    tauri::async_runtime::spawn(async move {
        let captured_output = (session_kind == GrokSessionKind::Review)
        ...
        let stdout_handle = if let Some(stdout) = stdout {
            ...
            let mut json_state = GrokJsonStreamState::default();
            ...
            Some(tokio::spawn(async move {
                ...
                while let Ok(Some(line)) = lines.next_line().await {
                    if let Some(parsed) = parse_grok_json_event_line(&line, &mut json_state) {
                        if let Some(session_id_value) = parsed.session_id {
                            bind_grok_session_id(
                                &app,
                                &emp_id,
                                t_id.as_ref(),
                                sk,
        ...
        let final_status = resolve_final_grok_status(current_status.as_deref(), exit_code);
        ...
        if let Ok(pool) = sqlite_pool(&app_clone).await {
            if session_kind == GrokSessionKind::Review {
...
mod tests {
    ...
    fn resolve_final_status_keeps_successful_executions_automation_eligible() {
        assert_eq!(
            resolve_final_grok_status(Some("running"), Some(0)),
            "exited"
        );
        assert_eq!(resolve_final_grok_status(None, Some(0)), "exited");
    }

    #[test]
    fn resolve_final_status_preserves_stopping_as_exited_without_success() {
        assert_eq!(
            resolve_final_grok_status(Some("stopping"), Some(143)),
            "exited"
        );
    }
    ...
    fn resolve_final_status_marks_failed_processes_failed() {
        assert_eq!(
            resolve_final_grok_status(Some("running"), Some(1)),
            "failed"
        );
        assert_eq!(resolve_final_grok_status(Some("running"), None), "failed");
    }
}
