...
fn normalize_scope(value: Option<&str>) -> Result<String, String> {
    match value.map(str::trim).unwrap_or(SCOPE_ALL) {
        ...
        SCOPE_PROJECTS => Ok(SCOPE_PROJECTS.to_string()),
        other => Err(format!("未知作用域：{other}，应为 all 或 projects")),
    }
}

fn normalize_project_ids(ids: &[String]) -> Vec<String> {
    let mut out = Vec::new();
    for id in ids {
        let id = id.trim();
        if id.is_empty() {
...
pub fn subagent_matches_project(item: &NativeSubagent, project_id: Option<&str>) -> bool {
    if item.scope != SCOPE_PROJECTS {
        return true;
    }
...
pub fn filter_native_subagents_for_project(
    items: &[NativeSubagent],
    project_id: Option<&str>,
) -> Vec<NativeSubagent> {
    items
        .iter()
        .filter(|item| subagent_matches_project(item, project_id))
        .cloned()
        .collect()
}
...
pub fn catalog_for_session(
    ...
    let mut out = filter_native_subagents_for_project(items, project_id);
...
fn normalize_record(mut item: NativeSubagent) -> Result<NativeSubagent, String> {
    ...
    if item.scope == SCOPE_ALL {
        item.project_ids = Vec::new();
    } else {
        item.project_ids = normalize_project_ids(&item.project_ids);
        if item.project_ids.is_empty() {
            return Err("指定项目时至少选择一个项目".to_string());
        }
    }
    Ok(item)
}
...
pub fn validate_native_subagent_id<R: Runtime>(
    ...
    if let Some(project_id) = project_id {
        if !subagent_matches_project(item, Some(project_id)) {
            return Err("该子智能体不适用于当前项目".to_string());
        }
    }
    Ok(Some(id.to_string()))
}

async fn validate_live_project_ids<R: Runtime>(
    app: &AppHandle<R>,
    ids: &[String],
) -> Result<(), String> {
    if ids.is_empty() {
    ...
    let live: Vec<String> =
        sqlx::query_scalar::<_, String>("SELECT id FROM projects WHERE deleted_at IS NULL")
            .fetch_all(&pool)
            .await
            .map_err(|error| format!("校验项目失败: {error}"))?;
    let live: HashSet<&str> = live.iter().map(String::as_str).collect();
    for id in ids {
        if !live.contains(id.as_str()) {
            return Err(format!("项目不存在或已删除：{id}"));
        }
    }
    Ok(())
}
...
pub async fn create_native_subagent<R: Runtime>(
    ...
    if record.scope == SCOPE_PROJECTS {
        validate_live_project_ids(&app, &record.project_ids).await?;
    }
...
pub async fn update_native_subagent<R: Runtime>(
    ...
    if let Some(project_ids) = payload.project_ids {
        next.project_ids = project_ids;
    }
...
mod tests {
    ...
    fn missing_scope_deserializes_as_all() {
        ...
        assert!(item.project_ids.is_empty());
    }

    #[test]
    fn projects_scope_requires_ids() {
        let mut item = sample();
        item.scope = SCOPE_PROJECTS.to_string();
        item.project_ids = Vec::new();
        let err = normalize_record(item).expect_err("empty projects");
        assert!(err.contains("至少选择一个项目"));
    }

    #[test]
    fn all_scope_clears_project_ids() {
        let mut item = sample();
        item.scope = SCOPE_ALL.to_string();
        item.project_ids = vec!["p1".to_string()];
        let out = normalize_record(item).unwrap();
        assert_eq!(out.scope, SCOPE_ALL);
        assert!(out.project_ids.is_empty());
    }
    ...
    fn filter_keeps_all_and_matching_projects() {
        ...
        scoped.scope = SCOPE_PROJECTS.to_string();
        scoped.project_ids = vec!["alpha".to_string(), "beta".to_string()];
        let items = vec![all.clone(), scoped.clone()];
        let for_alpha = filter_native_subagents_for_project(&items, Some("alpha"));
        assert_eq!(for_alpha.len(), 2);
        let for_other = filter_native_subagents_for_project(&items, Some("other"));
        assert_eq!(for_other.len(), 1);
        assert_eq!(for_other[0].id, "all");
        let free = filter_native_subagents_for_project(&items, None);
