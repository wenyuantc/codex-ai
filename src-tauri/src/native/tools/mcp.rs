...
use crate::app::{shell_escape_single_quoted, spawn_ssh_stdio_command, SshStdioProcess};
...
impl McpLiveServer {
    ...
    async fn shutdown(&mut self) {
        match &mut self.transport {
            ...
            McpTransport::Ssh { process, stdin, .. } => {
                let _ = stdin.shutdown().await;
                let _ = process.child.start_kill();
                process.cleanup_askpass();
            }
        }
    }
}
...
async fn spawn_remote<R: Runtime>(
    ...
    let mut process = spawn_ssh_stdio_command(app, ssh_config, &remote, true).await?;
    let stdin = process
        .child
        .stdin
        .take()
        .ok_or_else(|| "远程 MCP stdin 不可用".to_string())?;
    let stdout = process
        .child
        .stdout
        .take()
        .ok_or_else(|| "远程 MCP stdout 不可用".to_string())?;
    Ok(McpLiveServer {
        id: server.id.clone(),
        name: server.name.clone(),
        tools: Vec::new(),
