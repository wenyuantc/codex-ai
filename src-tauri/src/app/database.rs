...
pub fn get_ai_provider_capabilities() -> Vec<crate::db::models::AiProviderCapabilities> {
    vec![
        ...
        crate::db::models::AiProviderCapabilities {
            ...
            resume: true,
            mcp: true,
            notes: "支持启动/停止/续聊/会话中输入（进程内 mpsc 跟进）；重启=停止当前运行后重新启动。续聊从落库的模型历史恢复上下文（不恢复图片附件）。任务会话一轮结束后退出；自由会话可 send_native_input。按任务绑定注入 MCP 工具：本地本机 spawn，SSH 在远端 spawn（失败跳过，不回退本机）。".to_string(),
        },
    ]
}
...
mod ai_provider_capabilities_tests {
    ...
    fn capability_matrix_is_honest_and_complete() {
        ...
        assert!(
            by_provider["native"].resume,
            "native resume restores persisted model history"
        );
        assert!(
            by_provider["native"].notes.contains("落库")
                || by_provider["native"].notes.contains("上下文"),
            "native notes should document transcript resume"
        );
        assert!(
